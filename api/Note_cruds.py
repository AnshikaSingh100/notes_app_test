...
def getallnotes(request):
    ...
    regex = Notes.objects.all()
    for i in regex:
        if r in i.Readaccess.split(","):
            id_list.append(i.id)
    # id = {x for x in id_list}
    # id1=",".join(map(str,id_list))
    data1 = Notes.objects.filter(id__in=id_list)
    data = Notes.objects.filter(User=r)
    combined_qs = data | data1
    serializer = Noteserialiser(combined_qs, many=True)
